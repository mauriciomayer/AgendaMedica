...
- source_spec: `_bmad-output/implementation-artifacts/spec-6-3-minha-agenda-horario-sem-segundos.md`
  summary: `ScheduleBlock.startTime`/`endTime` continuam como `String` cru vindo do Postgres (`"HH:mm:ss"`) — a Story 6.3 corrigiu a formatação apenas no ponto de exibição em `MinhaAgendaScreen.kt` (`formatHora`), então qualquer tela futura que exiba esses campos diretamente pode reintroduzir o mesmo bug dos segundos aparecendo.
  evidence: Apontado no blind-hunter review da 6.3. Hoje só existe um ponto de exibição direta desses campos (`MinhaAgendaScreen.kt`, já corrigido); `DetalheMedicoScreen`/`DetalheMedicoViewModel` só usam `ScheduleBlock` para calcular horários disponíveis via `AgendaSlots.kt`, nunca para texto. Fechar isso estruturalmente exigiria tipar o campo como `LocalTime` (ou centralizar um formatter compartilhado) na camada de domínio/Repository — maior que uma correção cosmética de um único arquivo, fora do escopo que o Intent desta story definiu deliberadamente.

- source_spec: `_bmad-output/implementation-artifacts/spec-6-4-cancelamento-modal.md`
  summary: As duas linhas do I/O Matrix sobre o comportamento do `CancelConfirmDialog` ("toque fora não fecha o diálogo", "botão/gesto de voltar do sistema não fecha o diálogo") não têm nenhum teste automatizado cobrindo-as — dependem inteiramente de `DialogProperties(dismissOnBackPress = false, dismissOnClickOutside = false)`, verificado hoje só manualmente.
  evidence: Detectado no Matrix Test Audit do step-03. O projeto não tem `src/androidTest`/Compose-UI-test/Robolectric configurado (`app/build.gradle.kts:118-119` documenta essa ausência explicitamente), mesma limitação já registrada para a Splash Screen (spec-4-1). Fechar isso exigiria introduzir infraestrutura de teste de UI Compose para o projeto inteiro — desproporcional a uma única story de diálogo. Se uma história futura já trouxer essa infraestrutura por outro motivo, vale adicionar um teste de UI para este diálogo também.

- source_spec: `_bmad-output/implementation-artifacts/spec-6-5-mascara-email.md`
  summary: Comportamento do cursor/IME ao rejeitar uma tecla no meio do texto de um campo de e-mail (filtro em `onValueChange` sobre `OutlinedTextField` com `String`) não foi verificado — pode saltar para o fim do campo.
  evidence: Apontado por 3 revisores (blind-hunter e edge-case-hunter), veredito `maybe-false`; se ocorrer seria `medium` (atrapalha editar e-mail). Só se resolve no aparelho: digitar espaço no meio do texto de um e-mail e ver se o cursor fica no lugar; se saltar, migrar `LabeledTextField` para a sobrecarga `TextFieldValue` reposicionando a seleção.
